...
int main()
{
    int n, pr[30], p[30], bt[30], rt[30], ct[30], tat[30], wt[30];
    ...
    else
    {
        ...
        while (completed < n)
        {
            ...
            if (idx != -1)
            {
                ...
                if (rt[idx] == 0)
                {
                    ...
                    completed++;
                    ct[idx] = time;
                    tat[idx] = ct[idx]; // since arrival time is 0
        ...
        cout << "\n\nProcess\tpriority\tbrust\tcompletion\tturnaround\twating\n";
        for (int i = 0; i < n; i++)
        {
            cout << "p" << p[i] << "\t" << pr[i] << "\t\t"
